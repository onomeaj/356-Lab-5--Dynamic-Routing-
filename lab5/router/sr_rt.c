    ...
    while (list != NULL)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            arp_entry = sr_arpcache_lookup((&sr->cache), ip_hd->ip_dst);
          }
          else
          {
            arp_entry = sr_arpcache_lookup((&sr->cache), rt_match->gw.s_addr);
          }
          ...
          else
          {
            ...
            if (rt_match->gw.s_addr == 0)
            {
              sr_arpcache_queuereq(&sr->cache, ip_hd->ip_dst, packet, len, rt_match->interface);
            }
            else
            {
...
void sr_handleARP(struct sr_instance *sr, uint8_t *packet /* lent */, unsigned int len, char *interface /* lent */)
{
  ...
  if (arp->ar_op == htons(arp_op_request))
  {
    ...
    printf("creating arp response packet\n");
    ...
    memcpy(ether_hdr_resp->ether_shost, sr_if_resp->addr, 6);
    memcpy(ether_hdr_resp->ether_dhost, arp->ar_sha, 6);
    ...
    memcpy(arp_resp->ar_sha, sr_if_resp->addr, 6);
    memcpy(arp_resp->ar_tha, arp->ar_sha, 6);
    /*Send and free packet*/
    print_hdrs(outgoing_packet, len);
                     ...
                     char *interface /* lent */)
{
  ...
  printf("ln 446: packet type is %x\n", pkt_type);
